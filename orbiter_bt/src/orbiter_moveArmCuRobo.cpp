...
MoveArm_CuRobo::MoveArm_CuRobo(const std::string &name,
    ...
    node_(node),
    client(node->create_client<target_client::srv::TargetPose>("target_pose"))
{
...
BT::NodeStatus MoveArm_CuRobo::onStart()
{
    ...
    auto request = std::make_shared<target_client::srv::TargetPose::Request>();
...
void MoveArm_CuRobo::result_callback(rclcpp::Client<target_client::srv::TargetPose>::SharedFuture result)
{
